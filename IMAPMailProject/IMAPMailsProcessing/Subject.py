...
class FetchSubject():
    def __init__(self, userid, passwd, subject):
        self.userid = userid
        self.passwd = passwd
        self.subject = subject
    ################ IMAP SSL ##############################
    def SubjectProcess(self):
        with imaplib.IMAP4_SSL(host="imap.gmail.com", port=imaplib.IMAP4_SSL_PORT) as imap_ssl:
            ...
            resp_code, mails = imap_ssl.search(None, 'SUBJECT "'+ self.subject +'"')
            ...
            for mail_id in mails[0].decode().split():
                ...
                records.append({"fromaddress":message.get("From"),"subject": message.get("Subject"),"date": message.get("Date")})
